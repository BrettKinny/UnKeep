...
release_url=https://github.com/BrettKinny/UnKeep/releases/download/v0.2.0-rc.2
curl -fsSLO "$release_url/compose.release.yaml"
curl -fsSLO "$release_url/unkeep-0.2.0-rc.2-image-digest.txt"
...
release_digest="$(
  sed -n 's/^digest=\(sha256:[0-9a-f]\{64\}\)$/\1/p' \
    unkeep-0.2.0-rc.2-image-digest.txt
)"
...
```sh
gh release verify v0.2.0-rc.2 --repo BrettKinny/UnKeep
gh release verify-asset v0.2.0-rc.2 \
  compose.release.yaml --repo BrettKinny/UnKeep
gh release verify-asset v0.2.0-rc.2 \
  unkeep-0.2.0-rc.2-image-digest.txt --repo BrettKinny/UnKeep
gh release verify-asset v0.2.0-rc.2 SHA256SUMS --repo BrettKinny/UnKeep
```
...
```sh
git clone --branch v0.2.0-rc.2 https://github.com/BrettKinny/UnKeep.git
cd UnKeep
install -d -m 700 data
sudo chown 1000:1000 data
install -m 600 /dev/null .env
release_sha="$(git rev-parse HEAD)"
{
  printf 'UNKEEP_SETUP_TOKEN=%s\n' "$(openssl rand -base64 32)"
  printf 'UNKEEP_RECOVERY_TOKEN=%s\n' "$(openssl rand -base64 32)"
  printf 'UNKEEP_BUILD_VERSION=0.2.0-rc.2\n'
